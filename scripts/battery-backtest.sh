#!/usr/bin/env bash
# Replay the discharge history and score the runtime model against what
# actually happened.
#
...
# that measurement. It walks the history in order and, at each row, predicts
# that row's draw using only the rows before it — never the row itself — then
# reports the error of each candidate estimator against the observed draw.
#
# Estimators scored:
...
#
# Draw is what the model actually predicts; runtime is that draw divided into
# the stored energy, so an error in draw is the whole error. Scoring the draw
# keeps the harness independent of whatever the battery happened to hold.
#
# Usage: battery-backtest.sh [HISTORY_FILE]
# Exits 1 when there is too little history to hold anything out.
...
# Predictions only begin once there is at least this much prior evidence, so
# the first few rows do not score an estimator on almost nothing.
...
printf '  %d valid rows · %d scored predictions · warmup %d\n\n' \
  "$rows" "$scored" "$warmup"

if ((scored == 0)); then
  printf 'Not enough history to hold anything out. Collect more windows first.\n' >&2
  exit 1
fi
...
printf '\nA candidate estimator ships only when it beats "median" on mean and p50\n'
printf 'error here. "last" is the naive baseline: an estimator that cannot beat\n'
printf 'it is not learning anything.\n'
